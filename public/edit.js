...
const pharmacySelector = document.getElementById('pharmacySelector');
...
function loadPharmacies() {
  pharmacies.forEach(pharmacy => {
    ...
    pharmacySelector.appendChild(option);
  });
}

// When selected, fill the form
pharmacySelector.addEventListener('change', () => {
  const selectedName = pharmacySelector.value;
  const selectedPharmacy = pharmacies.find(p => p.name === selectedName);

  if (selectedPharmacy) {
    document.getElementById('name').value = selectedPharmacy.name || '';
    document.getElementById('email').value = selectedPharmacy.email || '';
    document.getElementById('valMonFri').value = '';
    document.getElementById('valSat').value = '';
    document.getElementById('valSun').value = '';
  } else {
    editForm.reset();
  }
});

// Handle update (log only, no API)
editForm.addEventListener('submit', (e) => {
  ...
  const updatedPharmacy = {
    ...
    values: {
      "Mon-Fri": document.getElementById('valMonFri').value.trim(),
      "Sat": document.getElementById('valSat').value.trim(),
      "Sun-Ph": document.getElementById('valSun').value.trim()
    }
  };

  console.log('Updated pharmacy data:', updatedPharmacy);

  responseDiv.innerHTML = `<div class="alert alert-success">Pharmacy "${updatedPharmacy.name}" updated locally (no database/API used).</div>`;
});
